...
observeDOM( observeMe ,function(){ 
    ...
    if (objCurItem != objSavedCurrentItem) {
		...
		if ("kan" in objCurItem)
		{
			...
			if (strQuestionType == "reading") {
				if(objCurItem.emph == "onyomi")
					strReadingType = strOn + strReading;
				else
					strReadingType = strKun + strReading;
			} else {
		...
		{
			case "en":
                var lvl = objCurItem.srs ? objCurItem.srs : '0';
				$('#question-type h1').html(strItemType + ' <strong>' + strReadingType + '</strong>' + ' lvl ' + lvl);
				break;
			case "ja":
