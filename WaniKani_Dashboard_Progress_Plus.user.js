...
//
// Hide the radicals/kanji in the progress circle.
...
//   localStorage.wkdpp_apprentice_only = 1;
//
// Hide brackets around 90%.
//   localStorage.wkdpp_hide_90percent = 1;
//
// Reverse the secondary date sort of items.
//   localStorage.wkdpp_reverse_date_sort = 1;
//
// Enable/Disable compact display (0=original, 1=compact)
//   localStorage.wkdpp_compact = 1;
//
// Configure time format (0 = 12hr (default), 1 = 24hr)
//   localStorage.wkdpp_timefmt = 1;
//
// Locked items sorted to front.
//   localStorage.wkdpp_locked_first = 1;
//===================================================================
...
window.wkdpp.progress_css =
    ...
    '.wkdpp-noshow {width:0px !important;}';

var css_compact =
    '.progression {padding-top:4px !important;padding-bottom:4px !important;}'+
    '.progression .chart {display:none;margin:0px !important;}'+
    '.progression h3 {font-size:14px !important;margin:4px !important;}'+
    '.progression hr {margin:0px !important;}'+
    '.progression .lattice-single-character {background-image:none !important;margin:0px 0px 5px 0px !important;}'+
    '.progression .lattice-single-character>div:first-child {display:none;margin:0px !important;}'+
    '.progression .lattice-single-character ul {display:block;margin:0px !important;}';
...
function get_api_key() {
    ...
    .done(function(page){
        ...
        // Extract the API key.
        var api_key = $(page).find('#user_api_key').attr('value');
...
function update_progress(type) {
    ...
    if (type==='radicals') {
        ...
        $.each(window.wkdpp.radical_data, function(idx,data){
            arr[data.meaning] = data;
        });
    } else {
        ul = $('.kanji-progress .lattice-single-character>ul');
        $.each(window.wkdpp.kanji_data, function(idx,data){
            arr[data.character] = data;
        });
    }
    ...
    ul.children().each(function(idx, li){
        ...
        if (type==='radicals') {
            var meaning = a.attr('href').split('/').slice(-1)[0];
            data = arr[meaning];
            if (data === undefined)
                data = {meaning: a.attr('data-original-title')};
        } else {
            var character = a.text();
            data = arr[character];
            if (data === undefined)
                data = {meaning: a.attr('data-original-title')};
        }
        li.data('api_data', data); // Store API data for use in sort below.
        ...
        var srs = (data && data.user_specific ? data.user_specific.srs_numeric : 10); // 10 == locked
        a.attr('data-srs-lvl', srs);

        // Populate the next review date.
        var next = (window.wkdpp.popup_date_only==1?'':'<br>');
        if (data && data.user_specific && data.user_specific.available_date) {
            if (data.user_specific.srs_numeric == 9) {
        ...
        if (type==='radicals') {
            if (window.wkdpp.popup_date_only==1)
                ...
                a.attr('data-original-title', a.attr('data-original-title')+next);
            if (data && data.user_specific) {
        ...
        if (window.wkdpp.blank_circles == 1) a.html('&nbsp;');
    });

    // Sort items by srs level, then review date, then meaning.
    var li_arr = ul.children();
    var srs_locked = (window.wkdpp.locked_first==1 ? -1 : 10);
    li_arr.sort(function(a,b){
        a = $(a).data('api_data');
        b = $(b).data('api_data');
        var a_srs = (a && a.user_specific ? a.user_specific.srs_numeric : srs_locked);
        var b_srs = (b && b.user_specific ? b.user_specific.srs_numeric : srs_locked);
        if (a_srs < b_srs) return -1;
        if (a_srs > b_srs) return 1;
        var a_avail = (a && a.user_specific && a.user_specific.available_date ?
            a.user_specific.available_date : Number.MAX_VALUE);
        var b_avail = (b && b.user_specific && b.user_specific.available_date ?
            b.user_specific.available_date : Number.MAX_VALUE);
        if (window.wkdpp.reverse_date_sort == 1) {
    ...
    if (window.wkdpp.apprentice_only == 1) {
        for (var srslvl=5; srslvl<=9; srslvl++) {
            $('.wkdpp-progress[data-srs-lvl="'+srslvl+'"]').parent().addClass('wkdpp-noshow');
        }
    }
    if (window.wkdpp.hide_90percent != 1) {
        ...
        var len = ul.children(':not(.wkdpp-noshow)').length;
        if (idx90 < len) {
            ul.children().eq(idx90).addClass('pct90_left');
            ul.children().slice(idx90).addClass('pct90');
            ul.children(':not(.wkdpp-noshow)').last().addClass('pct90_right');
        }
    }
}
...
function process_settings() {
    ...
    // Hide everything above Apprentice.
    window.wkdpp.apprentice_only = value_or_default(
        localStorage.wkdpp_apprentice_only,
        0
    );

    // Hide brackets around 90%.
    window.wkdpp.hide_90percent = value_or_default(
        localStorage.wkdpp_hide_90percent,
        0
    );

    // Reverse the secondary date sort of items.
    window.wkdpp.reverse_date_sort = value_or_default(
        localStorage.wkdpp_reverse_date_sort,
        0
    );

    // Enable compact display.
    window.wkdpp.compact = value_or_default(
        localStorage.wkdpp_compact,
        0
    );

    // Hide item information, i.e. only show SRS level and next review time.
    window.wkdpp.blank_circles = value_or_default(
        localStorage.wkdpp_blank_circles,
        0
    );

    // Configure time format (0 = 12hr (default), 1 = 24hr)
    window.wkdpp.timefmt = value_or_default(
        localStorage.wkdpp_timefmt,
        0
    );

    // Locked items sorted to front.
    window.wkdpp.locked_first = value_or_default(
        localStorage.wkdpp_locked_first,
        0
    );
}
...
function setup_styles() {
    ...
    addStyle(css);
    if (window.wkdpp.compact==1)
        addStyle(css_compact);
}
